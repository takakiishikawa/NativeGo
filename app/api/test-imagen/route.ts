...
async function tryModel(apiKey: string, model: string, method: string, body: unknown) {
...
export async function GET() {
  ...
  for (const model of ["imagen-4.0-fast-generate-001", "imagen-4.0-generate-001"]) {
    results[`${model}:generateImages`] = await tryModel(apiKey, model, "generateImages", {
  ...
  for (const model of ["gemini-2.5-flash-image", "gemini-3.1-flash-image-preview"]) {
    results[`${model}:generateContent`] = await tryModel(apiKey, model, "generateContent", {
