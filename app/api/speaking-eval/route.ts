...
スピーチ：${speechText || "(no speech detected)"}`
  ...
  const DEFAULT_COMMENT =
    "[GOOD]声に出す練習を続けていきましょう\n[UPGRADE]次回は短い文でも積極的に話してみましょう\n[GRAMMAR]文法を意識した発話に少しずつ慣れていきましょう"
  ...
  try {
    ...
    // 2nd attempt: sanitize literal newlines inside JSON string values, then re-parse
    try {
  ...
  // Final fallback: default evaluation so the user always gets a result
  if (
    !parsed ||
    typeof parsed !== "object" ||
    !Array.isArray((parsed as { scores?: unknown }).scores)
  ) {
    parsed = { scores: [1, 1, 1, 1, 1], total: 1, comment: DEFAULT_COMMENT }
  }

  evaluation = parsed as { scores: number[]; total: number; comment: string }

  // Normalise scores to integers (DB column is integer / smallint)
  const safeScores = evaluation.scores.map((s) => Math.round(Number(s) || 0))
  const safeTotal = Math.round(Number(evaluation.total) || 0)
